...
template <class T>
T wrap_angle_negPI_to_PI(T angle)
{
    int32_t multiples = (int32_t)(angle/PI);
    if (angle < 0)
        angle -= ((multiples-1)/2)*2*PI;
    else
        angle -= ((multiples+1)/2)*2*PI;
    return angle;
}

// Wrap angle (in radians) to be between (0, 2*PI]
template <class T>
T wrap_angle_0_to_2PI(T angle)
{
    int32_t revolutions = (int32_t)(angle/(2*PI));
    if (angle < 0)
        angle -= (revolutions-1)*2*PI;
    else
